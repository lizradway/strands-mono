...
export function ensureContextBenchCloned(): string {
  ...
  if (!existsSync(join(repoDir, '.git'))) {
    ...
    const lockFile = join(CACHE_DIR, 'cloning.lock')
    if (existsSync(lockFile)) {
      // Another process is cloning — wait for it
      ...
      try {
        ...
        try { execSync(`rm -f ${lockFile}`, { stdio: 'pipe' }) } catch {}
      }
    }
  }
...
export function ensureRepoCloned(task: ContextBenchTask): string {
  ...
  const lockFile = join(CACHE_DIR, `clone-${task.repo.replace('/', '__')}.lock`)

  // Clone repo (with lock for parallel safety)
  if (!existsSync(join(repoDir, '.git'))) {
    if (existsSync(lockFile)) {
      while (existsSync(lockFile)) {
        execSync('sleep 2', { stdio: 'pipe' })
      ...
      try {
        ...
        execSync(`git clone https://github.com/${task.repo}.git ${repoDir}`, {
          stdio: 'inherit',
        })
      } finally {
        try { execSync(`rm -f ${lockFile}`, { stdio: 'pipe' }) } catch {}
  ...
  // Fetch the specific commit (full clone so all commits available)
  // Use a worktree for the specific commit so parallel runs don't conflict
  const worktreeDir = join(CACHE_DIR, 'worktrees', `${task.repo.replace('/', '__')}__${task.baseCommit.slice(0, 12)}`)

  if (!existsSync(worktreeDir)) {
    mkdirSync(join(CACHE_DIR, 'worktrees'), { recursive: true })
    execSync(`git worktree add --detach ${worktreeDir} ${task.baseCommit}`, {
      cwd: repoDir,
      stdio: 'pipe',
    })
  }
